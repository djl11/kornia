...
class TestClose:
    ...
    def test_value(self, dev_str, dtype_str, call):
        ...
        expected = np.array([[0.7, 1., 0.8], [0.7, 0.7, 0.8], [0.7, 0.9, 0.8]])[None, None, :, :]
        assert np.allclose(call(close, input_, kernel), expected)
    ...
    def test_gradcheck(self, dev_str, dtype_str, call):
        ...
        assert gradcheck(close, (input_, kernel), raise_exception=True)
