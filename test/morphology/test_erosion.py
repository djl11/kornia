...
class TestErode:
    ...
    def test_smoke(self, device, dtype):
        dtype_str = ivy.torch.dtype_to_str(dtype)
        dev_str = ivy.torch.dev_to_str(device)
        for f, call in helpers.calls():
            with f.use:
                kernel = f.cast(f.random_uniform(shape=(3, 3), dev=dev_str), dtype_str)
    ...
    def test_value(self, device, dtype):
        ...
        for f, call in helpers.calls():
            with f.use:
                ...
                expected = np.array([[0.5, 0.3, 0.3], [0.3, 0.3, 0.2], [0.4, 0.2, 0.2]])[None, None, :, :]
    ...
    def test_gradcheck(self, device, dtype):
        ...
        for f, call in helpers.calls():
            ...
            with f.use:
                ...
                assert gradcheck(erosion, (input_, kernel), raise_exception=True)
