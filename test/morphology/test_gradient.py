...
class TestGradient:
    ...
    def test_value(self, dev_str, dtype_str, call):
        ...
        expected = np.array([[0.5, 0.7, 0.7], [0.4, 0.7, 0.6], [0.5, 0.7, 0.7]])[None, None, :, :]
        assert np.allclose(call(gradient, input_, kernel), expected)
    ...
    def test_gradcheck(self, dev_str, dtype_str, call):
        ...
        assert gradcheck(gradient, (input_, kernel), raise_exception=True)
